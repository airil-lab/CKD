...
hemo = st.number_input("Hemoglobin", min_value=3.1, max_value=17.8, step=1, value=10)
pcv = st.number_input("Packed Cell Volume / Hematocrit", min_value=9, max_value=54, value=30, step=1)
rc = st.number_input("Red Cells / RBC Count", min_value=2.1, max_value=8, value=6, step=1)
sc = st.number_input("Serum Creatinine", min_value=0.4, max_value=76, step=1, value=35)
sg = st.number_input("Specific Gravity of Urine", min_value=1, max_value=1.2364234115245358, step=0.1, value=1)
bgr = st.number_input("Blood Glucose Random", min_value=22, max_value=490, step=1, value=250)
al = st.number_input("Albumin", min_value=0, max_value=5, step=.1, value=2.5)
sod = st.number_input("Sodium - Na⁺", min_value=104, max_value=163, step=1, value=130)
pot = st.number_input("Potassium - K⁺", min_value=2.5, max_value=47, step=1, value=30)
bu = st.number_input("Blood Urea", min_value=1.5, max_value=391, step=1, value=150)
